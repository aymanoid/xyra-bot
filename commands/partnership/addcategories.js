...
class AddCategoriesCommand extends Command {
  constructor() {
    super('addcategories', {
      aliases: ['addcategories', 'addcategory'],
      description: {
        content: 'Adds categories.',
  ...
  async exec(msg, args) {
    ...
    const newCategoryIds = [
      ...new Set(oldCategoryIds.concat(validCategoryIds)),
    ];
    await this.client.settings.set(msg.guild, 'categoryIds', newCategoryIds);

    const categoriesEmbed = new MessageEmbed()
      .setColor(embedColor)
      .setTitle(`${EMOJIS.CHECKED} The following categories have been added:`)
...
export default AddCategoriesCommand;
